...
function 	printZlf(data){
	data=data.dataList;
	var target = document.getElementById('fangjianList')
	var html = ''
	for (var x = 0; x < data.length; x++) {
		html += '<option class="fjOption" value="' + data[x].id + '">' + data[x].mc + '</option>'
	}
	target.innerHTML += html		
	// 新增和修改窗口
	document.getElementById('m-fangjianList').innerHTML += html
...
function printTable(data) {
	...
	for (var x = 0; x < data.length; x++) {
	    ...
	    html += 	'<td>' + isnull(data[x].loucengMc) + '</td>';
	    html += 	'<td>' + isnull(data[x].fangjianMc) + '</td>';
	    html += 	'<td>' + isnull(data[x].mc) + ' </td>';
	    html += 	'<td>' + isnull(data[x].dz) + '</td>';
...
function  modify(id){
	...
	if(id){
		callAjax("zlfb/byid", {id:id}, function(data) {
			doModify(data.data);
		});		
	}
...
function doModify(data){
    ...
    $('#m-fangjianList').val(data.fang_jian);
